...
function getLineY({
  ...
  if (multiline) {
    ...
    const base = vCenter
      ? Math.round((intHeight - total) / 2) + intSize
      : intSize + 4;
...
const animations = {
  ...
  reveal(ctx) {
    ...
    const cursorX =
      (center ? (intWidth - lastLine.length * cw) / 2 : 20) +
      lastLine.length * cw +
      3;
    const cursorH = Math.round(intSize * 1);
    const cursorTop =
      lineY(lines.length - 1) - intSize + Math.round(intSize * 0.15);
    ...
    return `<defs><filter id="${fid}" x="-30%" y="-30%" width="160%" height="160%">
        ...
        </filter></defs>${charEls}<rect x="${cursorX.toFixed(1)}" y="${cursorTop}" width="4" height="${cursorH}" fill="${textColor}" filter="url(#${fid})" opacity="0">${cursorAnim}</rect>`;
  },
    ...
    return lines
      .map(
        (
          line,
          i,
        ) => `<text x="${textX}" y="${lineY(i)}" text-anchor="${textAnchor}" style="${commonStyle}" opacity="0">
            ${seqOp(i)}
    ...
    const items = lines
      .map(
        (
          line,
          i,
        ) => `<text x="${textX}" y="${lineY(i)}" text-anchor="${textAnchor}" style="${commonStyle}" filter="url(#${fb}_${i})" opacity="0">
            ${seqOp(i)}${line}
        </text>`,
      )
      .join("");
    ...
    return lines
      .map((_, i) => {
        ...
        for (let c = 0; c < Math.min(cols, 30); c++) {
          ...
          const delay = (
            i * ctx.slotS +
            (c / Math.max(cols, 1)) * lineDurS * 0.35
          ).toFixed(3);
    ...
    return lines
      .map(
        (
          line,
          i,
        ) => `<text x="${textX}" y="${lineY(i)}" text-anchor="${textAnchor}" style="${commonStyle}" opacity="0" transform-origin="${cx} ${lineY(i)}">
            ${seqOp(i)}
    ...
    return lines
      .map(
        (
          line,
          i,
        ) => `<text x="${textX}" y="${lineY(i)}" text-anchor="${textAnchor}" style="${commonStyle}" transform-origin="${cx} ${lineY(i)}" opacity="0">
            ${seqOp(i)}
      ...
      .map((line, row) => {
        ...
        return chars
          .map((ch, i) => {
            ...
            const begin = (
              row * ctx.slotS +
              (i / Math.max(chars.length, 1)) * lineDurS * 0.35
            ).toFixed(3);
...
module.exports = (req, res) => {
  ...
  const safeColor =
    (color || "36BCF7").replace(/[^a-fA-F0-9]/g, "").slice(0, 6) || "36BCF7";
  const safeBg =
    (background || "00000000").replace(/[^a-fA-F0-9]/g, "").slice(0, 8) ||
    "00000000";
  ...
  const seqOp = (i) =>
    seqOpacity({ index: i, slotS, lineDurS, cycleDurS, repeat: isRepeat });

  const bgRect =
    safeBg !== "00000000"
      ? `<rect width="${intWidth}" height="${intHeight}" fill="#${safeBg}"/>`
      : "";
  ...
  res.send(
    `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${intWidth}" height="${intHeight}" viewBox="0 0 ${intWidth} ${intHeight}">${bgRect}${content}</svg>`,
  );
};
